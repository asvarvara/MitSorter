...
- A BAM file which includes only non-methylated reads, in the *sorted_reads* folder
- A BAM file which includes only methylated reads, in the *sorted_reads* folder
...
<br/>
<br/>
The workflow uses the recently published complete human mitochondrial reference genome under GenBank accession number [CP068254.1](https://www.ncbi.nlm.nih.gov/nuccore/CP068254.1) (Nurk et al., 2022), and the latest available version of Dorado for the Conda environment (dorado-0.7.2, https://anaconda.org/HCC/dorado/files) for basecalling, together with the most recent modified basecalling models (*sup v5.0.0*) to achieve optimal accuracy for following variant calling. 
<br/>
